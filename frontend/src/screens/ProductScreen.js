...
import { addItem, deleteItem, saveCartDataToDB, updateItem } from "../actions/cartActions";
...
const ProductScreen = ({ history, match }) => {
  ...
  const addToCartHandler = () => {
      ...
      for (let i = 0; i < cartItems.length; i++) {
        if (cartItems[i].productId == parseInt(id)) {
          ...
          if (totalQuantity <= productDetails.products.countInStock) {
            updateItem(dispatch, id, totalQuantity);
            saveCartDataToDB(id, totalQuantity);
            return navigate(`/cart`);
          }
          else{
            return alert("You have selected more quantity than available stock!");
          }
        }
      }
    } else {
    ...
    addItem(dispatch, id, parseInt(oldQuantity) + parseInt(quantity));
    navigate(`/cart`);
  };
          ...
          <Row>
            ...
            <Col md={3}>
              <ListGroup variant="flush">
                ...
                <ListGroup.Item>
                  Price: {productDetails.products.price.toLocaleString("en-IN", {
                ...
                <ListGroup variant="flush">
                  <ListGroup.Item>
                    <Row>
                      <Col>Price:</Col>
                      <Col>
                        <strong>{productDetails.products.price.toLocaleString("en-IN", {
                            maximumFractionDigits: 2,
                            style: "currency",
                            currency: "INR",
                          })}</strong>
